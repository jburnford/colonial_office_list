#!/usr/bin/env python3
"""
Colonial Office List Batch Parser for 1931-1937
Final batch: Great Depression and Pre-WWII era
Includes Statute of Westminster (1931) and pre-war administrative changes
"""
...
# Colony names for 1931-1937 (1930s era)
KNOWN_COLONIES = {
    # Pre-existing colonies continuing from 1920s
    ...
    'BRITISH NEW GUINEA', 'BRITISH NORTH BORNEO', 'BRUNEI',
    ...
    'ZANZIBAR', 'ZULULAND',
    # League of Nations mandates (continuing from 1920s)
    'CAMEROONS', 'TOGOLAND', 'TANGANYIKA TERRITORY',
    'SOUTH WEST AFRICA', 'IRAQ', 'TRANSJORDAN',
    # Administrative reorganizations and 1930s additions
    'EAST AFRICA PROTECTORATE', 'KENYA COLONY', 'KENYA AND UGANDA',
    'NEW SOUTH WALES', 'NEW ZEALAND', 'SOUTH AUSTRALIA',
    'SOUTH AFRICA', 'UNION OF SOUTH AFRICA',
    # Possible dominion references (post-Statute of Westminster 1931)
    'DOMINION OF CANADA', 'COMMONWEALTH OF AUSTRALIA', 'DOMINION OF NEW ZEALAND',
    'IRISH FREE STATE',
}
...
SUBSECTION_PATTERNS = {
    ...
    'COMMUNICATIONS', 'HISTORY', 'DEFENCE', 'MILITARY',
}
...
class ColonialOfficeParser1931_1937:
    """Parser for 1931-1937 Colonial Office Lists from markdown"""
...
def process_year(year: int):
    ...
    parser = ColonialOfficeParser1931_1937(year)
    parser.load()
...
def main():
    """Process all years 1931-1937"""
    years = [1931, 1932, 1933, 1934, 1936, 1937]
    ...
    # Print summary
    print(f"\n{'='*70}")
    print("BATCH PROCESSING SUMMARY - 1931-1937 FINAL BATCH")
    print('='*70)
    total_colonies = 0
    for year, result in results.items():
        if result['status'] == 'success':
            total_colonies += result['colonies']
            print(f"{year}: ✓ {result['colonies']} colonies")
        else:
            print(f"{year}: ✗ ERROR - {result['message']}")

    print(f"\nTotal colonies extracted: {total_colonies}")
    print("This completes the final batch of Colonial Office Lists!")
