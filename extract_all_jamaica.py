#!/usr/bin/env python3
"""
Extract people data from ALL Jamaica Colonial Office List files.
Uses the Jamaica-specific extractor.
"""
...
from extract_jamaica_people import JamaicaExtractionOrchestrator


def find_all_jamaica_files():
    """Find all Jamaica files in output_3."""
    files = []

    for root, dirs, filenames in os.walk('output_3'):
        for filename in filenames:
            if 'jamaica' in filename.lower():
...
def main():
    print("="*80)
    print("JAMAICA COLONIAL OFFICE LISTS - COMPLETE EXTRACTION")
    print("="*80)
    print("\nUsing Jamaica-specific extractor (based on Ceylon v3 model)\n")

    # Find all files
    jamaica_files = find_all_jamaica_files()
    print(f"Found {len(jamaica_files)} Jamaica files\n")
    ...
    # Process each file
    for idx, (year, file_path) in enumerate(jamaica_files, 1):
        print(f"\n{'='*80}")
        print(f"[{idx}/{len(jamaica_files)}] Processing {year}: {os.path.basename(file_path)}")
        print('='*80)
    ...
    print(f"Files processed successfully: {len(year_stats)}/{len(jamaica_files)}")
    ...
    # Save results
    output_file = 'jamaica_all_years_v1.json'
    results = {
        'metadata': {
            'colony': 'JAMAICA',
