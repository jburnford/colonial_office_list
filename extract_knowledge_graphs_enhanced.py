#!/usr/bin/env python3
"""
Enhanced Knowledge Graph Extraction from Colonial Office Lists
With comprehensive personnel/people entity extraction
...
class ColonialOfficeExtractorEnhanced:
    def __init__(self, year):
        ...
        self.id_counter = 0
        self.seen_people = {}  # Track people to avoid duplicates
    ...
    def extract_people_from_civil_establishment(self, text, location):
        """
        Parse CIVIL_ESTABLISHMENT format:
        Position—Name, Honors.
        """
        lines = text.split('\n')
        ...
        for line in lines:
            ...
            if '—' in line:
                ...
                person_info = parts[1].strip()

                # Skip section headers and vacant positions at this stage
                if not person_info or person_info == "(Vacant)":
                    continue

                # Check if this is a list of multiple people (e.g., "Magistrates: A; B; C")
                if ':' in position_title and ';' in person_info:
                    # Handle multiple people in one line
                    base_title = position_title.split(':')[0].strip()
                    people_list = person_info.split(';')
                    for person in people_list:
                        self.extract_person_entry(person.strip(), base_title, location)
                else:
                    self.extract_person_entry(person_info, position_title, location)

    def extract_person_entry(self, person_info, position_title, location):
        """Extract a single person's information"""

        # Check for vacant position
        if '(Vacant)' in person_info or person_info == '(Vacant)':
            return

        # Parse honors (after comma)
        honors = []
        name = person_info
        if ',' in person_info:
            parts = person_info.rsplit(',', 1)
            name = parts[0].strip()
            honors_str = parts[1].strip()
            # Split honors by space, handling abbreviations like K.C.M.G., O.B.E., etc.
            honors = [h.strip() for h in re.split(r'\s+', honors_str) if h.strip()]

        # Parse titles from name (Sir, Rev., Dr., etc.)
        titles = []
        name_parts = name.split()
        remaining_parts = []

        title_prefixes = ['Sir', 'Rev.', 'Rev', 'Dr.', 'Dr', 'Prof.', 'Prof', 'Lady', 'Lord', 'Major', 'Colonel', 'Admiral', 'General']

        for part in name_parts:
            if part in title_prefixes or part.endswith('.') and len(part) <= 4:
                titles.append(part)
            else:
                remaining_parts.append(part)

        clean_name = ' '.join(remaining_parts).strip()

        if not clean_name:
            return

        # Create unique key to avoid duplicates
        person_key = f"{clean_name}_{position_title}_{location}"

        if person_key in self.seen_people:
            # Update existing person with additional position
            person_id = self.seen_people[person_key]
            # Find and update the person entity
        ...
        self.seen_people[person_key] = person_id
        ...
        if titles:
            person_entity["titles"] = titles

        if honors:
            person_entity["honors"] = honors

        person_entity["positions"] = [{
            "title": position_title,
            "location": location,
            "year": self.year,
            "status": "permanent"
        ...
        self.entity_ids["person"].append(person_id)
    ...
    def extract_demographics(self, text, colony_name):
        ...
        demo_entity["breakdowns"] = breakdowns_found[:10]
        ...
        for file_path in colony_files:
            ...
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    # For civil establishment files, extract people
                    if 'CIVIL_ESTABLISHMENT' in colony_name or 'STAFF' in colony_name:
                        self.extract_people_from_civil_establishment(text, colony_name)
                    else:
    ...
    def create_output(self, colonies_processed):
        ...
        output = {
            "metadata": {
                ...
                "processing_notes": f"Enhanced extraction from {len(colonies_processed)} colony files with comprehensive people/personnel entity recognition",
    ...
    def extract(self):
        ...
        print(f"Processed {len(colonies_processed)} colonies/territories")
...
def main():
    ...
    for year in target_years:
        extractor = ColonialOfficeExtractorEnhanced(year)
    ...
    print(f"\n{'='*60}")
    print("COMPREHENSIVE EXTRACTION SUMMARY REPORT")
    print(f"{'='*60}")
    ...
    print("\n" + "="*60)
    if all_files_valid:
        print("SUCCESS: All extraction files created successfully!")
    else:
        print("WARNING: Some extraction files were not created")
    print("="*60)
